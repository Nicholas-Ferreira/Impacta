...
def fibonacci(n, lista = [0, 1]):
  ...
  if len(lista) == n:
    return lista
  return fibonacci(n, lista + [lista[-1]+lista[-2]])

#print(f'Fatorial: {fatorial(10)}')
#print(f'Soma de Array: {somaArray([12, 3, 20, 18])}')
...
False se nao é
'''
#  [ [ [1, segredo], 0, 2 ], [2, 1, 0, 0], 4, 3, 3, 4, 2 ]
def acha_segredo(lista):
  if 'segredo' in lista or lista == True:
    return True
  for l in lista:
    if type(l) == list:
      if acha_segredo(l):
